...
for file_name in os.listdir(path_read):     # 读入所有等待被处理的文件
    ...
    if operator.eq(file_name[-4:], 'xlsx') == 1:    # 判断这个文件是不是xlsx文件，如果是，则进行下面的操作（通过这个步骤过滤掉其他后缀的文件以及该目录下的文件夹）
        ...
        print(parameter_df.head())  # 打印前几行，查看结构
...
for model_dataset, file_set in file_dict.items():    # 对于每一组model-dataset进行处理
    ...
    changed_para_str = ', '.join(changed_para)  # 将改变了的参数的参数名连成字符串，用于生产输出文件的文件名+changed_para_str+'_'
    path_write = path + '/data_collection'+ '/' + model_dataset + '_' + str(int(time.time())) + str(int(random.uniform(100, 900))) + '.xlsx'
    # 创建并保存工作簿
    wb = Workbook()
    ...
    save_value([[para_df, 'Parameters']], path_write, first_sheet=False)  # 将所有出现过的参数名和参数值存下来
