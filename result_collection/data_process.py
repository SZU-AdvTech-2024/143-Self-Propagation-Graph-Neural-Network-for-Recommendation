...
print(f"Reading from: {path_read}")
...
for file_name in os.listdir(path_read):             # 读入所有等待被处理的文件
    if operator.eq(file_name[-5:], '.xlsx') == 1:           # 判断这个文件是不是xlsx文件，如果是，则进行下面的操作（通过这个步骤过滤掉其他后缀的文件以及该目录下的文件夹）
        ...
        parameter_df = pd.DataFrame(pd.read_excel(file_p, sheet_name=0, header = None, engine='openpyxl'))  # 读入待处理文件的第一个sheet（这个sheet里存储着实验参数），header=None表明在这个表格中并没有表头
...
for key, value in file_dict.items():                    # dict.items方法会将键-值对作为元组返回
    ...
    path_write = path_read + '/data_process/' + dataset + '_' + model + '_' + str(int(time.time())) + str(int(random.uniform(100, 900))) + '.xlsx'  # 输出文件的文件路径
    ...
    # 如果没有有效的工作表，先创建一个有效的工作表
    if not wb.sheetnames:
        wb.create_sheet("Parameters")  # 创建一个名为 "Data" 的工作表
    ...
    save_value([[parameter, 'Parameters']], path_write, first_sheet=False)  # 将参数存进excel
